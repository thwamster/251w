...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\noindent Challenge Problem \vspace{0.3cm}

	Let $f: A \rightarrow B$ be a function.

	\begin{enumerate}
		\item Prove that for any subset $X \subseteq A$, the following inclusion holds:
		\[ X \subseteq f^{*}(f_{*}(X)) \]
		Identify the specific property $f$ must possess such that $X = f^{*}(f_{*}(X))$ for all $X \subseteq A$.

		\medskip

		\item Prove that for any subset $Y \subseteq B$, the following inclusion holds:
		\[ f_{*}(f^{*}(Y)) \subseteq Y \]
		Identify the specific property $f$ must possess such that $f_{*}(f^{*}(Y)) = Y$ for all $Y \subseteq B$.
	\end{enumerate}
